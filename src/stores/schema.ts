...
export const useSchemaStore = defineStore('schema', () => {
  ...
  function orderTablesForSqlExport(tables: Table[], relations: Relation[]): Table[] {
    ...
    for (const relation of relations) {
      ...
      const dependents = dependentsByTableId.get(relation.sourceTableId)!
      if (dependents.has(relation.targetTableId)) continue

      dependents.add(relation.targetTableId)
      dependencyCountByTableId.set(
        relation.targetTableId,
        (dependencyCountByTableId.get(relation.targetTableId) ?? 0) + 1,
      )
    }
  ...
  function exportSQL(dialect: SQLDialect = schema.value.dialect ?? 'postgresql'): string {
    ...
    const orderedTables = orderTablesForSqlExport(s.tables, s.relations)
