  ...
  <aside class="sidebar">
    ...
    <div class="sidebar-section">
      ...
      <div class="btn-row three">
        <button class="btn-ghost-sm" @click="newProject" title="New project">⊕ New</button>
        <button class="btn-ghost-sm" @click="store.saveToFile()" title="Save .dbm.json">↓ Save</button>
        <label class="btn-ghost-sm" title="Load .dbm.json">
          ↑ Load
    ...
    <div class="sidebar-section flex-grow">
      ...
      <div class="item-list scrollable">
        <div
          ...
          :title="`${table.name}${table.groupId ? ' · ' + groupName(table.groupId) : ''}`"
        >
          <span class="item-dot" :style="{ background: table.color }" />
          <span class="item-name">{{ table.name }}</span>
          <span v-if="table.groupId" class="item-group-tag" :style="{ color: groupColor(table.groupId) }">
    ...
    <div v-if="store.selectedTable && !store.selectedGroupId" class="sidebar-section inspector">
      ...
      <!-- Group assignment -->
      <label class="section-label" style="margin-top:8px">Group</label>
      <select class="field-select" v-model="tableGroupId" @change="onChangeTableGroup">
    ...
    <div v-if="store.selectedRelationId && !store.selectedTable && !store.selectedGroup" class="sidebar-section inspector">
      ...
      <button class="btn-ghost-action danger" style="width:100%;margin-top:8px" @click="store.deleteRelation(store.selectedRelationId!)">
        ✕ Delete Relation
      </button>
    </div>
